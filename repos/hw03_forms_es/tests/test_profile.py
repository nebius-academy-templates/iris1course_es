...
class TestProfileView:
    ...
    def test_profile_view_get(self, client, post_with_group):
        try:
            ...
            assert False, f'''La página `/profile/<username>/` no se descarga adecuadamente. Error: `{e}`'''
        ...
        assert len(page_context.object_list) == 1, (
            'Asegúrate de haber agregado los artículos del autor correcto al contexto de página de `/profile/<username>/`'
        )
