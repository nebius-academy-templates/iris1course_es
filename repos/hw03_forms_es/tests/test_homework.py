...
class TestPost:
    ...
    def test_post_admin(self):
        ...
        assert hasattr(admin_model, 'empty_value_display'), (
            'Agrega un valor por defecto `-empty-` para los campos vacíos'
        )
...
class TestGroupView:
    ...
    def test_group_view(self, client, post_with_group):
        try:
            ...
            assert False, f'''La página `/group/<slug>/` no se descarga adecuadamente. Error: `{e}`'''
        if response.status_code in (301, 302):
            response = client.get(f'/group/{post_with_group.group.slug}/')
        if response.status_code == 404:
            assert False, 'La página `/group/<slug>/` no se ha encontrado, verifica esta ruta en *urls.py*'
