...
class TestGroupPaginatorView:
    ...
    def test_group_paginator_view_get(self, client, few_posts_with_group):
        ...
        if response.status_code in (301, 302):
            response = client.get(f'/group/{few_posts_with_group.group.slug}/')
        assert response.status_code != 404, 'La página `/group/<slug>/` no se ha encontrado, verifica esta ruta en *urls.py*'
    ...
    def test_index_paginator_view(self, client, post_with_group):
        response = client.get('/')
        assert response.status_code != 404, 'La página `/` no se ha encontrado, verifica esta ruta en *urls.py*'
        assert 'page_obj' in response.context, (
