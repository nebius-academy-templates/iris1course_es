...
class TestCreateView:
    ...
    def test_create_view_get(self, user_client):
        try:
            ...
            assert False, f'''La página `/create/` no se carga adecuadamente. Error: `{e}`'''
        ...
        assert response.context['form'].fields['text'].required, (
            'Asegúrate de que el formulario `form` en la página `/create/` tiene un campo obligatorio `text`'
        )
    ...
    def test_create_view_post(self, user_client, user, group):
        ...
        assert post is not None, 'Asegúrate de guardar la nueva publicación cuando la forma correspondiente sea enviada en la página `/create/`'
