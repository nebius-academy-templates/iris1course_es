...
try:
    ...
    assert False, 'No se ha encontrado el modelo Group'
...
class TestPost:
    ...
    def test_index_view(self, client, post_with_group):
        try:
            ...
            assert False, f'''La página principal no se carga correctamente. Error: `{e}`'''
        assert response.status_code != 404, (
            'La página main (principal) no se encuentra, verifica esta ruta en *urls.py*'
        )
        assert response.status_code != 500, (
            'No se carga la página principal. Comprueba la función de visualización correspondiente'
        )
        assert response.status_code == 200, (
            'La página principal no se carga correctamente.'
        )
        # Modelos de prueba
        ...
        for p in posts:
            assert p.text in response_text, (
                'Asegúrate de que la página principal muestra todas las publicaciones '
                'ordenadas por fecha de publicación de más reciente a más antigua'
            )
    ...
    def test_post_model(self):
        ...
        assert pub_date_field is not None, 'Añade la fecha y hora del evento: el campo `pub_date` del modelo `Post`'
        assert type(pub_date_field) == fields.DateTimeField, (
            'El atributo `pub_date` del modelo `Post` debe ser de tipo `DateTimeField`'
        )
        ...
        assert group_field is not None, 'Añade un atributo `group` al modelo `Post`'
        ...
        assert group_field.related_model == Group, (
            'El campo `group` del modelo `Post` debe ser una `Group`, una referencia a otro modelo'
        )
        assert group_field.blank, (
            'El campo `group` del modelo `Post` debe tener el parámetro `blank=True`'
        )
        assert group_field.null, (
            'El campo `group` del modelo `Post` debe tener el parámetro `null=True`'
        )
    ...
    def test_post_admin(self):
        ...
        assert Post in admin_site._registry, 'Registra el modelo `Post` en el panel de administración'
        ...
        assert 'group' in admin_model.list_display, (
            'Muestra `group` en la interfaz de administración'
        )
        assert 'pk' in admin_model.list_display, (
            'Muestra `pk` en la interfaz de administración'
        )
        assert 'text' in admin_model.search_fields, (
            'Añade una opción para buscar mediante `text` en la interfaz de administración'
        )

        assert 'group' in admin_model.list_editable, (
            'Habilita la edición del campo `group` en la interfaz de administración'
        )

        assert 'pub_date' in admin_model.list_filter, (
            'Añade un filtro por `pub_date` en la interfaz de administración'
        )
        ...
        assert admin_model.empty_value_display == '-empty-', (
            'Add a default value `-empty-` para los campos vacíos'
        )
...
class TestGroup:
    ...
    def test_group_model(self):
        ...
        assert title_field is not None, 'Añade el nombre del evento `title` al modelo `Group`'
        assert type(title_field) == fields.CharField, (
            'El atributo `title` del modelo `Group` debe ser de tipo `CharField`'
        )
        assert title_field.max_length == 200, 'Establece la longitud máxima de `Group` del modelo de `title` en 200'

        slug_field = search_field(model_fields, 'slug')
        assert slug_field is not None, 'Añade un slug `slug` al modelo `Group`'
        ...
        assert description_field is not None, 'Añade una descripción `description` del modelo `Group`'
    ...
    def test_group_create(self, user):
        ...
        slug = 'test-link'
        description = 'Descripción de grupo de prueba'
...
class TestGroupView:
    ...
    def test_group_view(self, client, post_with_group):
        ...
        assert search_refind(r'{%\s*endfor\s*%}', html_template), (
            'Edita la plantilla HTML, no se encuentra el atributo de cierre de bucle'
        )

        assert re.search(
            r'<\s*p(\s+class=".+"|\s*)>\s*' + post_with_group.text + r'\s*<\s*\/p\s*>',
            html
        ), 'Edita la plantilla HTML, no se encuentra el cuerpo de la publicación `<p>{{ post_body }}</p>`'

        assert re.search(
            r'(d|D)ía de publicación:\s*',
            html
        ), (
            'Edita la plantilla HTML, no se encuentra la fecha de publicación '
            '`Día de publicación: {{ publication_date|date:"d E Y" }}`'
        )

        assert re.search(
            r'(a|A)utor\:\s' + post_with_group.author.get_full_name(),
            html,
        ), (
            'Edita la plantilla HTML, no se encuentra el autor '
            '`Autor: {{ full_name_of_the_author }},`'
        )

        base_template = get_template('base.html').template.source
        assert re.search(
            r'{\%\sload static\s\%}', base_template
        ), 'Carga datos estáticos en la plantilla base.html'
